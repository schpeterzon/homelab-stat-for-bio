...
func Load(path string) (Config, error) {
	...
	set("repository_path", &c.RepositoryPath)
...
func (c *Config) normalize(base string) error {
	if c.Proxmox.TokenSecret == "" {
		c.Proxmox.TokenSecret = os.Getenv("PROXMOX_TOKEN_SECRET")
	}
	if c.RepositoryPath == "" {
		c.RepositoryPath = "."
	}
...
func boolValue(v map[string]string, key string, fallback bool) bool {
	if s, ok := v[key]; ok {
...
func parseYAML(input string) (map[string]string, error) {
	...
	for s.Scan() {
		...
		if indent > len(sections) {
			return nil, fmt.Errorf("invalid indentation near %q", clean)
		}
		if indent < len(sections) {
			sections = sections[:indent]
		}
		parts := strings.SplitN(clean, ":", 2)
		if len(parts) != 2 {
			return nil, fmt.Errorf("invalid YAML line %q", clean)
		}
